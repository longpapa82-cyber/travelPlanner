...
const ProfileScreen = ({ navigation }: any) => {
  ...
  return (
    <View style={styles.outerContainer}>
      ...
      {/* isAdDebugAllowed = isServiceAdmin OR temp diagnosis allow-list (2026-06-09). */}
      {(isServiceAdmin || isAdDebugAllowed) && (
        <View style={styles.section}>
          <Text style={styles.sectionTitle}>{t('sections.admin')}</Text>
          {/* Revenue dashboard stays service-admin-only — temp Ad Debug accounts must NOT see it. */}
          {isServiceAdmin && (
