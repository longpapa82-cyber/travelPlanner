...
## 📍 현재 상태 (2026-05-03 KST)

### 핵심 상태
- **버전**: versionCode 220 — **프로덕션 검토 중** ✅ (177개 국가, 18,405 기기)
- **서버**: https://mytravel-planner.com (Hetzner VPS) — V220 backend 배포 완료 ✅
- **브랜치**: `main`
- **Frontend**: TypeScript 0 errors, Jest 230/230 PASS
- **Backend**: TypeScript 0 errors, Jest 416/416 PASS (단, auth.service.spec / auth.controller.spec / subscription.service.spec 3개 suites는 otplib ESM 이슈로 pre-existing failure — V220과 무관)
- **자동 검증**: `npm run validate:static` PASS

### 최근 버전 이력
| 버전 | 날짜 | 핵심 수정 |
|------|------|-----------|
| **V220(프로덕션 검토 중)** | 2026-05-03 | OAuth CSRF nonce 보안 강화 + AsyncStorage refresh token 제거 + CSP unsafe-inline 제거 + 17개 언어 법적 문서 업데이트 (Apple Refresh Token, Cloudflare, FCM) |
...
### 다음 작업 (우선순위 순)
1. **V220 프로덕션 검토 통과 확인** — 자동 게시 대기
2. **APK 로컬 테스트** — `build-v220.apk` (96MB)로 디바이스 검증
   - A1: 카카오 로그인 정상 동작
   - A2: 카카오 취소 메시지 ("카카오 로그인이 취소되었습니다")
   - A3: 구글 로그인 정상 동작
   - A4: 회원탈퇴 후 카카오 재가입 (duplicate key 에러 없는지)
   - A5: 개인정보처리방침 → Apple Refresh Token, Cloudflare, FCM 항목 확인
   - A6: 구독 플로우 정상 동작
3. **Git commit** — V220 변경사항 전체 커밋
4. **초기 리뷰/평점 확보** — 지인 테스터 리뷰 요청
5. **Android 15/16 deprecated API** — `edgeToEdgeEnabled: true` + StatusBar 마이그레이션 + `screenOrientation` 제한 삭제 (V221)
...
**최종 업데이트**: 2026-05-03 KST — V220(versionCode 220) 프로덕션 검토 중. OAuth CSRF nonce + AsyncStorage 보안 + CSP 강화 + 17개 언어 법적 문서 업데이트. 다음: 검토 통과 후 자동 게시 → APK 로컬 테스트 → Git commit.
