...
- **버전**: versionCode 43 (Alpha 테스트 대기 중)
- **서버**: https://mytravel-planner.com (Hetzner VPS)
- **상태**: P0 핫픽스 빌드 중 → Alpha 테스트 예정

### 최근 배포 (versionCode 43, 2026-03-30)
- **수정 내용**: 6개 Critical UX 버그 수정 (P0 핫픽스)
- **배포 상태**: 🔄 빌드 중 (EAS)
- **다음 단계**: Alpha 트랙 배포 → 1-2일 테스트 → 프로덕션 출시

### P0 핫픽스 (versionCode 43, 2026-03-30)
- **Bug #6 (P0)**: 중복 여행 생성 방지 (useRef 가드, race condition 수정)
- **Bug #3, #4 (P1)**: 위치 자동완성 선택 반영 (stale closure 수정)
- **Bug #2 (P2)**: 날짜 라벨 수정 ("도착일" → "종료일")
- **Bug #5 (P2)**: Android 네비게이션 바 버튼 겹침 (SafeAreaInsets)
- **Bug #1 (P1)**: 광고 보상 기능 피드백 추가 (Toast 알림)
- **배포 상태**: ✅ 커밋 완료, 🔄 빌드 진행 중
