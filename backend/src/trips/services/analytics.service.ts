...
export class AnalyticsService {
  ...
  async getPopularDestinations(limit: number = 10): Promise<DestinationStats[]> {
    ...
    try {
      ...
      for (const [destination, data] of destinationMap.entries()) {
        ...
        // 평균 여행 인원
        const totalTravelers = data.trips.reduce((sum, trip) => sum + trip.numberOfTravelers, 0);
  ...
  async getUserPreferenceStats(): Promise<UserPreferenceStats> {
    ...
    try {
      ...
      const averageDuration = trips.length > 0 ? Math.round(totalDuration / trips.length) : 0;

      // 평균 여행 인원
      const totalTravelers = trips.reduce((sum, trip) => sum + trip.numberOfTravelers, 0);
      const averageTravelers = trips.length > 0 ? Math.round(totalTravelers / trips.length) : 0;
      ...
      this.logger.error(`Failed to get user preference stats: ${error.message}`);
      return {
        budgetDistribution: {},
        travelStyleDistribution: {},
        interestDistribution: {},
  ...
  async getDestinationRecommendations(destination: string): Promise<{
    ...
    try {
      ...
      const recommendedDuration = Math.round(totalDuration / relevantTrips.length);

      // 평균 여행 인원
      const totalTravelers = relevantTrips.reduce((sum, trip) => sum + trip.numberOfTravelers, 0);
      const recommendedTravelers = Math.round(totalTravelers / relevantTrips.length);
      ...
      this.logger.error(`Failed to get destination recommendations: ${error.message}`);
      return {
        recommendedDuration: 5,
        recommendedTravelers: 2,
        bestMonths: [],
